...
@Aspect
@Component
public class ElecDeviceAspect implements ApplicationEventPublisherAware {
	...
	@Pointcut("execution(* com.homw.gateway.device.impl.*ElecDeviceImpl.open(..))")
	public void open() {
	}

	@Pointcut("execution(* com.homw.gateway.device.impl.*ElecDeviceImpl.close(..))")
	public void close() {
	}

	@Pointcut("execution(* com.homw.gateway.device.impl.*ElecDeviceImpl.search(..))")
	public void search() {
	}
	...
	public Pair<Boolean, String> open(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		try {
			...
			if (pair.getLeft()) {
				...
				redisDeviceInfoService.redisDeleteDeviceInfo(deviceInfo.getOuterNo(),
	...
	public Pair<Boolean, Double> search(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		if (pair.getLeft()) {
			if (pair.getRight() != null) {
				...
				redisDeviceInfoService.redisDeleteDeviceInfo(deviceInfo.getOuterNo(), Constant.DeviceType.ELECTRIC.name());
				// 计算使用度数，比如公摊
